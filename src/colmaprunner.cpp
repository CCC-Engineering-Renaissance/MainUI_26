...
void ColmapRunner::setDenseEnabled(bool enabled)
{
    m_denseEnabled = enabled;
}
...
void ColmapRunner::maybeAppendOption(QStringList &args,
...
void ColmapRunner::runFullPipeline()
{
    ...
    QStringList featureArgs = {"feature_extractor",
                               "--database_path",
                               dbPath,
                               "--image_path",
                               m_imagePath,
                               ...
                               "8192"};
    maybeAppendOption(featureArgs, "feature_extractor", "--SiftExtraction.use_gpu", "1");
    maybeAppendOption(featureArgs, "feature_extractor", "--SiftExtraction.gpu_index", "0");

    QStringList matchingArgs = {"sequential_matcher",
                                "--database_path",
                                dbPath,
                                ...
                                "1"};
    maybeAppendOption(matchingArgs, "sequential_matcher", "--SiftMatching.use_gpu", "1");
    maybeAppendOption(matchingArgs, "sequential_matcher", "--SiftMatching.gpu_index", "0");

    m_steps = {
        {"Feature Extraction", "", featureArgs},

        {"Feature Matching", "", matchingArgs},
...
#if defined(Q_OS_WIN)
    if (m_denseEnabled) {
        QStringList denseStereoArgs = {"patch_match_stereo",
                                       "--workspace_path",
                                       densePath,
                                       "--workspace_format",
                                       "COLMAP",
                                       ...
                                       "1"};
        maybeAppendOption(denseStereoArgs, "patch_match_stereo", "--PatchMatchStereo.use_gpu", "1");
        maybeAppendOption(denseStereoArgs,
                          "patch_match_stereo",
        ...
        m_steps.append({"Dense Stereo (CUDA)", "", denseStereoArgs});
    ...
    if (m_denseEnabled) {
        ...
        m_steps.append(
            {"Prepare OpenMVS Input", "sh", {"-c", QString("cp %1/0/* %1/").arg(sparsePath)}});

        m_steps.append({"Convert to OpenMVS",
                        "InterfaceCOLMAP",
                        {"--input-file",
                         m_workspacePath,
                         "--image-folder",
        ...
        m_steps.append(
            {"Clean Old Depth Maps",
             "sh",
             {"-c", QString("rm -f %1/depth*.dmap %1/dense*.mvs %1/*.ply").arg(m_workspacePath)}});
...
void ColmapRunner::startStep(const PipelineStep &step)
{
    ...
    if (step.name == "Sparse Reconstruction") {
        emit progressOutput("[info] COLMAP mapper is CPU-only. GPU acceleration applies to SIFT "
                            "extraction/matching and dense stereo.");
    } else if (step.name == "Stereo Fusion") {
        emit progressOutput("[info] stereo_fusion is CPU-bound even when dense stereo used CUDA.");
    }
    ...
    if (!m_process->waitForStarted(5000)) {
        emit errorOccurred(QString("Failed to start '%1': %2").arg(exe, m_process->errorString()));
